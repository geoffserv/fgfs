...
// [x][y][2]=1 - Loop group. 4 button group of loop buttons. [x][y][3] tracks activated state
const byte NUM_OF_SWITCHVALUE_S1_GROUP = 5;
const byte SWITCHVALUE_S1_GROUP[NUM_OF_SWITCHVALUE_S1_GROUP][2] = { {0, 0}, {0, 1}, {0, 2}, {0, 3}, {1, 3} };
// [x][y][2]=2 - Individual Pushbutton toggle. [x][y][3] tracks activated state
const byte NUM_OF_SWITCHVALUE_S2_GROUP = 4;
const byte SWITCHVALUE_S2_GROUP[NUM_OF_SWITCHVALUE_S2_GROUP][2] = { {2, 0}, {2, 1}, {2, 2}, {2, 3} };
// [x][y][2]=3 - Fade group. [x][y][3] tracks activated state
const byte NUM_OF_SWITCHVALUE_S3_GROUP = 3;
const byte SWITCHVALUE_S3_GROUP[NUM_OF_SWITCHVALUE_S3_GROUP][2] = { {1, 0}, {1, 1}, {1, 2} };
//
...
void setup() {
  ...
  for (byte i=0; i<NUM_OF_SWITCHVALUE_S1_GROUP; i++) {
    switchValue[SWITCHVALUE_S1_GROUP[i][0]][SWITCHVALUE_S1_GROUP[i][1]][2] = 1; // Set to switchType 1 - Loop group
  }
  ...
  // Initializing Third group - Fade group
  for (byte i=0; i<NUM_OF_SWITCHVALUE_S3_GROUP; i++) {
    switchValue[SWITCHVALUE_S3_GROUP[i][0]][SWITCHVALUE_S3_GROUP[i][1]][2] = 3; // Set to switchType 3 - Fade group
  }
...
void loop() {
  ...
  for (byte xpin = 0; xpin < NUM_OF_X_PINS; xpin++) { // Loop through every digital x pin
              ...
              if (switchValue[xpin][ypin][3] == 1) { // If the state is OFF
                ...
                for (byte i=0; i<NUM_OF_SWITCHVALUE_S1_GROUP; i++) { // loop through each of the switches in the switchgroup
                  if (switchValue[SWITCHVALUE_S1_GROUP[i][0]][SWITCHVALUE_S1_GROUP[i][1]][3] == 0) { // if the switch is on
                    ...
                    setButton(SWITCHVALUE_S1_GROUP[i][0], SWITCHVALUE_S1_GROUP[i][1], HIGH, 1, 0); // Turn off the switch, pulse midi off
                  }
                }
                // Turn on this switch
                setButton(xpin, ypin, LOW, 0, 1); // Turn on the switch, Pulse midi on
    ...
    if (cycleCount == BLINKCYCLE) { // if we have looped around enough for a blink
      for (int i = 0; i < NUM_OF_SWITCHVALUE_S3_GROUP; i++) { // loop through each of the blinkgroup buttons
        if (switchValue[SWITCHVALUE_S3_GROUP[i][0]][SWITCHVALUE_S3_GROUP[i][1]][3] == 0) { // if the switch is on
          // Invert the LED statuses
          if (switchValue[SWITCHVALUE_S3_GROUP[i][0]][SWITCHVALUE_S3_GROUP[i][1]][1] == HIGH) {
            switchValue[SWITCHVALUE_S3_GROUP[i][0]][SWITCHVALUE_S3_GROUP[i][1]][1] = LOW;
          }
          else {
            switchValue[SWITCHVALUE_S3_GROUP[i][0]][SWITCHVALUE_S3_GROUP[i][1]][1] = HIGH;
          }
        }
      }
      cycleCount = 0;
