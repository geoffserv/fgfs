...
const byte NUM_OF_Y_PINS = 8;
const byte SWITCHPINS_X[NUM_OF_X_PINS] = { 0, 1, 2, 3, 4, 5, 6, 7 };  // pins to which the x row is connected
const byte SWITCHPINS_Y[NUM_OF_Y_PINS] = { 8, 9, 10, 11, 12, 13, 14, 15 }; // pins to which the y col is connected
...
const int MIDI_NOTE_NUMS[NUM_OF_SWITCHES] = {40, 41, 42, 43, 36, 37, 38, 39, 44};
    ...
    for (byte ypin = 0; ypin < NUM_OF_Y_PINS; ypin++) { // Loop through every digital y pin
      ...
      if ((switchValue[n] != switchCurrentValue)) { // If there's an edge change ) { // Oh button n's state has changed from our last recorded state!
        ...
        if (switchCurrentValue == 0) { // Switch ON!
          usbMIDI.sendNoteOn (40, 110, MIDI_CHAN);
        }
        else { // Switch OFF!
          usbMIDI.sendNoteOff (40, 0, MIDI_CHAN);
        }
      }
