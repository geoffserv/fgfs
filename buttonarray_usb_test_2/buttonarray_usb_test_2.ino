  ...
  for (byte i = 0; i <= 2; i++) { // Loop through every digital pin
    ...
    for (byte j = 0; j <= 2; j++) { // Loop through each pin again,
      if (j != i) { // If it's not the same as the output pin (e.g. only input pins pls,)
        ...
        if ((switchValue[n] != switchCurrentValue) && (buttonPins[SWITCHPIN[j]].fallingEdge() || buttonPins[SWITCHPIN[j]].risingEdge())) { // If there's an edge change ) { // Oh button n's state has changed from our last recorded state!
          switchValue[n] = switchCurrentValue; // Record the new value
          char buf[22];
          sprintf(buf, "Button %d value is %d", n, switchValue[n]);
          Serial.println(buf);
        }
